...
- Summarized golden metrics experienced by an individual user
...
Once you have selected a user, you will presented with a screen showing an overview of that user's experience with the application for the active time period.

This overview contains two sections:

- golden metrics pertinent to that entity type, giving you a quick understanding of the user's overall quality of experience
...
_A note about Browser sessions: Session Timeline relies on the [Browser agent session](https://docs.newrelic.com/docs/browser/browser-monitoring/page-load-timing-resources/cookie-collection-session-tracking/) as the attribute that connects related browser events together. The New Relic session can be very long-lived, spanning across multiple days. Session Timeline will timebox long-lived sessions into daily chunks, to help keep the amount of data considered in the timeline view meaningful. In this way, you may see the same session id repeated multiple times in the table - that simply means that the user's browser was not closed in that timeframe._
...
In the example below, we can scan the list easily to see how the user navigated through the site, as well as when errors were encountered.
...
Each event is grouped into a category based on the underlying data, and the timeline chart represents the events based on their grouping. Clicking on the chart legend allows for rapid filtering of the both the events shown in the chart, as well as the in the listing below.
...
Many of the parameters used by Session Timeline can be adjusted in the Configuration screen. To access this screen, click on the gears icon in the upper right corner of the Overview screen.

![Open Configuration Screenshot](docs/screenshots/config_01.png)<br/>

#### Configuration Scope <a id="scope"></a>

The configuration settings are specific to the **entity**, but are common to **all users**: if you make a change, all users in the account will see that change, but they will be applied only to the active entity (not to all entities of that type).
...
- `root event` <a id="root-event"></a>
  ...
  It is not recommended to change this value, unless you are not using the Browser SPA agent. In this case, set the `root event` to `PageView`.
...
- `timeline event types` <a id="timeline-event-types"></a>
  ...
  The set of event types that will be included when building the [timeline view](#timeline). The available events are restricted to include only event types relevant to the application (Browser of Mobile event types).
  ...
  If you are not using the Browser SPA agent, it is recommended that you deselect `BrowserInteraction` and select `PageView`

- `timeline event titles` <a id="timeline-event-titles"></a>

  The `timeline event titles` allow you to determine which attribute should be displayed at the top level of a timeline event item.
...
- `event thresholds` <a id="event-thresholds"></a>
  ...
  You can define multiple threshold rules for each event type. An event threshold definition consists of:
...
Note that any changes made to `packDefaults` will not be overwrite the active configuration for any entities that have already gone through the first time set up.
